...
Eighteen read tools over the 376-endpoint API, plus four write tools that appear
only when `CASHCTRL_MODE=write`. See [PLAN.md](PLAN.md) for the design and the
full list of Stolpersteine it works around.
...
user**. The key is scoped to one organisation and inherits the role you give it
...
are only needed for the directory documents and backups land in — drop them both
if you never use `download_document` or `create_backup`.
...
376 endpoints do not fit in a tool list, so this is eighteen curated read tools
plus a searchable escape hatch for everything else. Four write tools are
registered as well when `CASHCTRL_MODE=write`, and are absent otherwise.
...
| Tool                 | What it does                                          |
| -------------------- | ----------------------------------------------------- |
| `create_record`      | Creates one record of any writable resource           |
| `update_record`      | Changes named fields, preserving every field left out |
| `delete_record`      | Deletes records by id                                 |
| `book_journal_entry` | Books a double entry, naming accounts by number       |

All four **preview by default**: called without `confirm: true` they return the
exact request they would send and change nothing.

`update_record` is the one that earns its place over `call_api`. CashCtrl's
update endpoints are full replacements — "all parameters must be submitted,
omitted parameters are treated as empty values" — so posting `person/update`
with `{ id, lastName }` renames the person _and_ clears their address, category,
notes and everything else. `update_record` reads the record first and resends
its current values, so only what you name actually changes. The writable field
list is the update endpoint's own parameter list from the vendored index, so it
stays right as the API moves.

`book_journal_entry` takes account **numbers** rather than ids, refuses a date
outside any fiscal period (CashCtrl answers such a date with a silent 0 rather
than an error) and refuses a closed period.
...
model to establish the fiscal period before reading anything that depends on it,
and to name write steps as tasks rather than attempt them.

## The change history

`history/list.json` is documented as taking only `orderId`, `personId` and
`statementId`, which undersells it. On a live organisation it covers **26 entity
types** — orders, book entries, salary statements, journal imports, accounts,
tax rates, fiscal periods, files, master data — with seven change types
(`CREATE`, `UPDATE`, `DELETE`, `STATUS`, `IMPORTED`, `DOWNLOAD`, `SEND`), and
the generic `filter` array works on `created`, `type`, `changeType` and
...
returns end amounts as positive magnitudes per account class, so an _open_
period shows `Aktiven − Passiven = Ergebnis`, while a _closed_ one shows
...
new version of a record only when its contents actually change. Measured against
the organisation this was built on:
...
_deletion_, and an unknown filter field is silently ignored rather than
...
between two runs, each field's `from` and `to`, and — with `entityId` — the full
version history of one record. `lastUpdated` is ignored, since it moves whenever
anything else does.
...
- **Generated invoice PDFs are excluded on purpose.** Fetching one appends a
  ...
  record a backup exists to preserve. Reading file _contents_ was measured and
...
- **A date in no fiscal period returns `0`, not an error.**
  `get_account_balance` refuses such a date and lists the periods that do exist.
...
- **Report rows carry 30-odd display fields per node**, half of them
  ...
  guessing.
- **A booking with no tax code is usually fine.** Only 3 of 147 accounts in the
  organisation this was built against define a default tax code, so
  `review_bank_import` flags a missing one only where the contra account itself
  expects it, and reports the rest as an aggregate rather than 60 rows of noise.
- **Entries staged by an import but never booked are invisible in the journal.**
  They are counted on the import side and reported separately.
- **Executing a bank import can close customer invoices.** CashCtrl matches
  incoming payments against open invoices when the import is _created_, and the
  matched entry carries the target status. `review_pending_import` lists which
  invoices an execute would close, and with what status, before it happens.
...
Uploading bank statements **is** possible through the API even though nothing
here does it yet: `file/prepare` returns pre-authenticated URLs, the bytes go up
with a plain `PUT`, `file/persist` saves them, and `journal/import/create`
stages the entries. CashCtrl accepts camt.052, camt.053, camt.054, MT940, Excel
and CSV, including inside a ZIP, up to 5 MB. Nothing is booked until
`journal/import/execute` runs, so the staging half is comparatively safe — but
it still writes.
...
should match exactly as the UI does.

The SDK's write suite now drives the whole chain — `file/prepare`, the `PUT`,
`file/create`, `journal/import/create`, the per-entry confirm/unconfirm, and
`journal/import/execute` — against a disposable organisation, so the mechanics
are proven. What is not yet proven is the _matching_: that an API import closes
the same invoices the UI would. Until it is, upload in the UI and use
`review_pending_import` to check the matches before executing.
